  ...
  constructor() {
    ...
    this.requests = {
      fetchTasksFilteredByPost: () =>
        superagent.post(base_url_public + '/tasks/fetchtasks/filterbypost', { user_token: reactLocalStorage.get('loggedToken') }).then(res => {
          if (!res.body.result) {
          ...
          tasks_datas.length === 0 ?
            <div>
              ...
              <div className="no_tasks_title">
                {lang.no_tasks_posted_yet}!
              </div>
              <div className="no_tasks_subtitle">
                { lang.no_task_description }
              </div>
