...
export default class TasksCancelled extends Component {
  constructor() {
    ...
    this.requests = {
      fetchTasksFilteredByCancelled: () =>
        superagent.post(base_url_public + '/tasks/fetchtasks/filterbycancelled', { user_token: reactLocalStorage.get('loggedToken') }).then(res => {
          if (!res.body.result) {
  ...
  componentDidMount() {
    this.requests.fetchTasksFilteredByCancelled();
  }
      ...
      <div key={"react_list_key" + index} className="tasklist_item">
        <div className="tasklist_item_real">
          ...
          <div className="tasklist_item_desccontainer_taskposted">
            ...
            <div className="task_title6 redtext">
              {lang.tasks_cancelled}
            </div>
          </div>
